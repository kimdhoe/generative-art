...
const sketch = () => {
  return ({ context, width, height }) => {
    ...
    for (let i = 0; i < t; i++) {
      for (let j = 0; j < t; j++) {
        const f = random.pick([ drawCell1, drawCell2 ])
        f(context, cellWidth * i, cellHeight * j, cellWidth * (i + 1), cellHeight * (j + 1), true)
      }
    }
  }
}
...
const drawCell2 = (context, ax, ay, bx, by) => {
  const rotate = random.rangeFloor(0, 4)
  const centerX = ax + (bx - ax) / 2
  const centerY = ay + (by - ay) / 2
  ...
  context.translate(-(bx - ax) / 2, (bx - ax) / 2)
  context.beginPath()
  context.arc(centerX, centerY, (bx - ax) / 2, Math.PI * -0.5, 0)
  context.lineWidth = LINE_WIDTH
  context.lineCap = 'round'
  context.strokeStyle = 'black'
  context.stroke()
